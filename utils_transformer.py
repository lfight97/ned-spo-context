...
class BinaryProcessor(DataProcessor):
    ...
    def get_dev_examples(self, data_dir):
        ...
        return self._create_examples(
            ...
            # 测试集用
            #self._read_tsv(os.path.join(data_dir, "dev_test.tsv")), "dev")
    ...
    def _create_examples(self, lines, set_type):
        """Creates examples for the training and dev sets."""
        """
        对输入进行调整，将query侧(新闻desc)作为text_a，同时引入英文得desc
        和Qid放在一起，暂时不处理triplets乱序问题
        """
        examples = []
        for (i, line) in enumerate(lines):
            guid = line[0]
            #text_a = line[1]
            text_a = line[2]
            if '_|_' not in line[1]:
                print(guid, line[1])
                raise AssertionError
            text_b = line[1].replace('_|_',',')
            #text_b = line[1].split('_|_')[0]            
            text_c = line[3]
            # 测试时没有triplets时候的性能
            #text_c = ''
            label = line[4]
            if len(line) == 6 and len(line[5])>5:
                text_c = line[5]
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=text_b, text_c=text_c, label=label))
